    ...
    try {
      ...
      if (isBrowser) {
        try {
          ...
          // Check if a description was already generated by the server
          if (result && !result.description && !result._fallback) {
            try {
              // If not, try to generate one using the data we got back
              console.log('Generating description from processed receipt data...');
              const description = await generateReceiptDescription(result);
              ...
              console.warn('Failed to generate description:', descError);
