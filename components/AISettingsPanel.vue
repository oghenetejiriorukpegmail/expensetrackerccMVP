...
async function testReceiptDescription() {
  ...
  try {
    // First try Document AI processing to extract receipt data
    let documentAiData = null;
    let description = null;
    
    // Process the receipt with Document AI first
    try {
      console.log('Processing receipt with Document AI...');
      const processResponse = await fetch('/.netlify/functions/process-receipt', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          imageData: receiptImage.value
        })
      });
      
      if (processResponse.ok) {
        const processData = await processResponse.json();
        documentAiData = processData.receipt;
        // Check if description was already generated by the server
        if (processData.description) {
          description = processData.description;
          console.log('Description already generated by server:', description);
        }
      } else {
        console.warn('Document AI processing failed:', await processResponse.text());
      }
    } catch (processError) {
      console.warn('Error processing with Document AI:', processError);
    }
    
    // If we have Document AI data but no description yet, request description directly
    if (documentAiData && !description) {
      console.log('Document AI successful, requesting description with extracted data...');
      try {
        ...
        if (descriptionResponse.ok) {
          ...
          console.warn('Description generation failed:', await descriptionResponse.text());
        }
      } catch (descError) {
        console.warn('Error generating description:', descError);
      }
    }
    
    // Fallback: If Document AI or description generation failed, try the original flow
    if (!documentAiData || !description) {
      console.log('Using fallback for receipt testing due to Document AI/description failures');
      ...
      // Use the data from our improved flow
      receiptResult.value = {
        success: true,
        extractedData: documentAiData,
        description: description,
        message: 'Successfully processed receipt and generated description'
      };
    }
  } catch (error) {
